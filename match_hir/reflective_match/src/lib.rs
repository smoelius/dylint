...
use match_hir::{__snippet_opt as snippet_opt, __span as span, Error, Pattern};
...
impl<'tcx> LateLintPass<'tcx> for ReflectiveMatch {
    fn check_expr_post(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        // smoelius: Ignore "no-location" spans (i.e., spans with no associated source file) and
        // "from expansion" spans.
        let Some(span) = span(cx.tcx, expr.hir_id) else {
            self.ignored.push(expr.span);
            return;
        };

        // smoelius: `matches` returns the "most ancestral" `HirId`. So if `expr`'s parent has the
        // same span as `expr`, ignore `expr`.
        if cx
            .tcx
            .hir()
            .span(cx.tcx.parent_hir_id(expr.hir_id))
            .source_equal(span)
        {
        ...
        let snippet = snippet_opt(cx, span).unwrap();

        if snippet.is_empty() {
            self.errors.push(Error::other(span, EmptySnippet));
            return;
        }
